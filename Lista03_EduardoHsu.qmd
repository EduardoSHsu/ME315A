...
São equivalentes visto que ambas se tratam de, sendo C as condições, C1 AND (C2 OR C3) = (C1 AND C2) OR (C1 AND C3) e de (C1 AND C2) OR (C1 AND C3).
...
São equivalentes visto que ambas se tratam de, sendo C as condições, C1 OR (C2 AND C3) = (C1 OR C2) AND (C1 OR C3) e de (C1 OR C2) AND (C1 OR C3)
...
Visto que, sendo C as condições, na primeira linha temos NOT (C1 AND C2) = (NOT C1) OR (NOT C2), isso é equivalente à terceira linha, já a segunda linha é diferente pois ela apenas funciona apenas quando as todas as condições são atendidas, enquanto as outras duas funcionam para qualquer uma das condições.
...
Visto que, sendo C as condições, na primeira linha temos NOT (C1 OR C2) = (NOT C1) AND (NOT C2), isso é equivalente à terceira linha, já a segunda linha é diferente funciona para qualquer condição, já as outras duas apenas quando todas condições são atendidas.
