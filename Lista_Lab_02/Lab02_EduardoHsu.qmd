...
As estatística suficientes são o número total de voos e o número total de voos com atrasos na chegada maior que 10 minutos.
...
```{r, message=FALSE}
# Nota: Lendo o arquivo "flights.csv" diretamente, pois o Posit Cloud 
# realiza a extração automática do .zip no ambiente virtual.
dados_parciais <- read_csv_chunked("flights.csv", 
                  callback = DataFrameCallback$new(getStats), 
                  chunk_size = 100000, 
                  col_types = cols_only(DAY = col_integer(), 
                                        MONTH = col_integer(), 
...
computeStats <- function(dados_parciais){
  ...
  summarise(atraso_total = sum(voos_atraso),
            total_geral = sum(voos_total),
            Perc = atraso_total/total_geral
  ) %>%
  mutate(Data = as.Date(paste("2015", MONTH, DAY, sep="-"))) %>%
  select(Cia = AIRLINE, Data, Perc)
}

estatisticas_finais <- computeStats(dados_parciais)
```
...
baseCalendario <- function(stats, cia) {
  dados_calendario <- stats %>% 
    filter(Cia == cia) %>%
    select(Data, Perc) 
  
  
    calendario <- ggcal(dados_calendario$Data, dados_calendario$Perc)
...
cAA + pal + ggtitle("Mapa de Calor da porcentagem de voos com atraso maior 
                    de 10 minutos da Companhia Aérea AA")
cDL + pal + ggtitle("Mapa de Calor da porcentagem de voos com atraso maior 
                    de 10 minutos da Companhia Aérea DL")
cUA + pal + ggtitle("Mapa de Calor da porcentagem de voos com atraso maior 
                    de 10 minutos da Companhia Aérea UA")
cUS + pal + ggtitle("Mapa de Calor da porcentagem de voos com atraso maior 
                    de 10 minutos da Companhia Aérea US")
```
