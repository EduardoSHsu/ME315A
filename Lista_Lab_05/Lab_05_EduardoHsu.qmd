...
analisa_aeronave <- function(tail_number, arquivo) {
  ...
  # A função callback apenas filtra. O read_csv_chunked 
  # com DataFrameCallback cuida de unificar os resultados.
  callback <- function(x, pos) {
    x %>% filter(TAIL_NUMBER == tail_number)
  }
  
  # A função já retorna o data frame unificado
  ...
  # Ler a base de aeroportos (lendo de fora do arquivo zip)
  # O dirname(arquivo) garante que vai procurar na mesma pasta em que o zip está[cite: 2, 3]
  caminho_airports <- file.path(dirname(arquivo), "airports.csv")
  airports <- read_csv(caminho_airports)
  
  # Juntar coordenadas de origem e destino preservando todas as colunas do df original
  voos <- voos %>%
  ...
  # Estatística adicional: velocidade média (mph)
  voos <- voos %>%
    mutate(velocidade_media = DISTANCE / (AIR_TIME/60)) 
  
  # Criar mapa leaflet[cite: 3]
  mapa <- leaflet(voos) %>%
  ...
  # Retorno da função com a lista nomeada[cite: 2]
  return(list(
    tabela = voos,
    grafico = mapa
  ))
...
# Ver a tabela tidy
resultado$tabela %>%
  head(10) %>%  # só para não estourar o relatório HTML
